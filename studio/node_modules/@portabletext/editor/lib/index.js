...
function validateSelection(slateEditor, editorElement) {
  ...
  try {
    ...
    debug.selection("Could not resolve selection, selecting top document"), Transforms.deselect(slateEditor), slateEditor.children.length > 0 && Transforms.select(slateEditor, Editor.start(slateEditor, [])), slateEditor.onChange();
  }
}
  ...
  $[64] !== editorActor || $[65] !== propsSelection || $[66] !== slateEditor ? (t13 = () => {
    if (propsSelection) {
      ...
      if (normalizedSelection !== null) {
        debug.selection(`Normalized selection from props ${JSON.stringify(normalizedSelection)}`);
        const slateRange = toSlateRange({
          context: {
            schema: editorActor.getSnapshot().context.schema,
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      if (Editor.isEditor(node) && node.children.length === 0 && withoutPatching(editor, () => {
        ...
        for (const [child, childPath] of children) {
          ...
          if (editor.isTextSpan(child) && editor.isTextSpan(nextNode) && child.marks?.every((mark) => nextNode.marks?.includes(mark)) && nextNode.marks?.every((mark) => child.marks?.includes(mark))) {
            debug.normalization("merging spans with same marks"), withNormalizeNode(editor, () => {
      ...
      if (editor.isTextBlock(node) && !Array.isArray(node.markDefs)) {
        debug.normalization("adding .markDefs to block node"), withNormalizeNode(editor, () => {
      ...
      if (defaultStyle && editor.isTextBlock(node) && typeof node.style > "u") {
        debug.normalization("adding .style to block node"), withNormalizeNode(editor, () => {
          Transforms.setNodes(editor, {
            style: defaultStyle
          }, {
            at: path
      ...
      if (editor.isTextSpan(node) && !Array.isArray(node.marks)) {
        debug.normalization("Adding .marks to span node"), withNormalizeNode(editor, () => {
          Transforms.setNodes(editor, {
            marks: []
          }, {
            at: path
      ...
      if (editor.isTextSpan(node)) {
        ...
        if (editor.isTextBlock(block) && node.text === "" && annotations && annotations.length > 0) {
          debug.normalization("removing annotations from empty span node"), withNormalizeNode(editor, () => {
            Transforms.setNodes(editor, {
        ...
        for (const [child, childPath] of Node.children(editor, path))
          if (editor.isTextSpan(child)) {
            ...
            if (orphanedAnnotations.length > 0) {
              debug.normalization("removing orphaned annotations from span node"), withNormalizeNode(editor, () => {
                Transforms.setNodes(editor, {
      ...
      if (editor.isTextBlock(node)) {
        ...
        if (markDefs.length !== newMarkDefs.length) {
          debug.normalization("removing duplicate markDefs"), withNormalizeNode(editor, () => {
            Transforms.setNodes(editor, {
      ...
      if (editor.isTextBlock(node) && !editor.operations.some((op) => op.type === "merge_node" && "markDefs" in op.properties && op.path.length === 1)) {
        ...
        if (node.markDefs && !isEqualMarkDefs(newMarkDefs, node.markDefs)) {
          debug.normalization("removing markDef not in use"), withNormalizeNode(editor, () => {
            Transforms.setNodes(editor, {
              markDefs: newMarkDefs
            }, {
              at: path
      ...
      if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === editorActor.getSnapshot().context.schema.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
        ...
        if (editor.isTextBlock(targetBlock)) {
          ...
          debug.normalization("copying markDefs over to merged block", op), Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
            at: targetPath,
            voids: !1
...
function createPatchesPlugin({
  ...
  return function(editor) {
    ...
    return subscriptions.push(() => {
      ...
      return () => {
        debug.syncPatch("unsubscribing to remote patches"), sub.unsubscribe();
      };
...
function updateValuePlugin(context, editor) {
  ...
  return editor.apply = (operation) => {
    if (editor.isNormalizingNode && debug.normalization(`(slate operation)
...
function transformOperation(editor, patch, operation, snapshot, previousSnapshot) {
  ...
  if (patch.type === "insert" && patch.path.length === 1) {
    ...
    return debug.history(`Adjusting block path (+${patch.items.length}) for '${transformedOperation.type}' operation and patch '${patch.type}'`), [adjustBlockPath(transformedOperation, patch.items.length, insertBlockIndex)];
  }
  if (patch.type === "unset" && patch.path.length === 1) {
    ...
    return "path" in transformedOperation && Array.isArray(transformedOperation.path) && transformedOperation.path[0] === unsetBlockIndex ? (debug.history("Skipping transformation that targeted removed block"), []) : [adjustBlockPath(transformedOperation, -1, unsetBlockIndex)];
  }
  if (patch.type === "unset" && patch.path.length === 0)
...
function performEvent({
  ...
  mode === "send" && !isNativeBehaviorEvent(event) && (editor.undoStepId = defaultKeyGenerator()), debug.behaviors(`(${mode}:${eventCategory(event)})`, JSON.stringify(event, null, 2));
  ...
  if (eventBehaviors.length === 0 && isSyntheticBehaviorEvent(event)) {
    nativeEvent?.preventDefault(), mode === "send" && (editor.undoStepId = void 0), withPerformingBehaviorOperation(editor, () => {
      debug.operation(JSON.stringify(event, null, 2)), performOperation({
        context: {
          keyGenerator,
          schema
        },
    ...
    typing: {
      ...
      states: {
        idle: {
          ...
          exit: [() => {
            debug.mutation("exit: typing->idle"), debug.mutation("entry: typing->typing");
          }],
          on: {
            typing: {
              target: "typing"
...
const syncValueCallback = ({
  ...
  guards: {
    ...
    "is busy": ({
      ...
      return debug.syncValue(JSON.stringify({
        isBusy,
...
function syncBlock({
  ...
  })) : (debug.syncValue("Replacing block", oldBlock, block), Editor.withoutNormalizing(slateEditor, () => {
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        replaceBlock({
          context,
...
function updateBlock({
  ...
  if (Transforms.setNodes(slateEditor, slateBlock, {
    ...
    slateBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - slateBlock.children.length)).forEach((_, i) => {
      ...
      childIndex > 0 && (debug.syncValue("Removing child"), Transforms.removeNodes(slateEditor, {
        at: [index, childIndex]
      }));
      ...
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key && currentBlockChild._type === oldBlockChild?._type) {
          ...
          isSpanNode2 && isTextChanged ? (oldBlockChild.text.length > 0 && deleteText(slateEditor, {
            ...
            at: path
          }), slateEditor.onChange()) : isSpanNode2 || (debug.syncValue("Updating changed inline object child", currentBlockChild), Transforms.setNodes(slateEditor, {
            _key: VOID_CHILD_KEY
          }, {
            at: [...path, 0],
            voids: !0
          }));
        } else oldBlockChild ? (debug.syncValue("Replacing child", currentBlockChild), Transforms.removeNodes(slateEditor, {
          at: [index, currentBlockChildIndex]
        }), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [index, currentBlockChildIndex]
        }), slateEditor.onChange()) : oldBlockChild || (debug.syncValue("Inserting new child", currentBlockChild), Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [index, currentBlockChildIndex]
        }), slateEditor.onChange());
    });
  }
